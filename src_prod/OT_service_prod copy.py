...
class OT_service:
    ...
    def read_solutions(self, solutions_df):
        ...
        for reaction, volume in sum_of_reactions.items():
                print('reaction', reaction, 'volume', volume, 'ml')
    ...
    def check_time_temperature_and_shaker(self):
        ...
        for i, row in self.solutions.iterrows():
            if np.isnan(row['time']) == False:
                ...
                elif row['time'] not in self.times and self.times == []:
                    self.times.append(row['time'])
...
"\n",
f"def custom_transfer(pipette,tip_rack, volume, source, destination, blow_out=True, blowout_location='destination well', air_gap=0):",
...
f"   tip_rack = protocol.load_labware('{self.tip_rack}', {self.tip_rack_slot})",
f"   pipette = protocol.load_instrument('p1000_single', 'right', tip_racks=[tip_rack])"]
        ...
        for j, row in solutions.iterrows():
            if component == row['component']:
                ...
                while leftover-0.01 > 0:
                    ...
                    if source['volumes'][i]+0.01 >= leftover:
                        v=leftover * 1000
                    ...
                    if len(source['rows']) == 1 and count_components == 1 and v>10:
                        ...
                        self.ot_functions.append(f"   custom_transfer(pipette=pipette,tip_rack=tip_rack, volume={v},source={s},destination={d}, blow_out={bool(self.blowout)}, air_gap={self.air_gap})")
                        #first and not last
                    elif dispense_index == 0 and (len(source['rows'])>1 or count_components > component_index+1) and v>10:
                        self.ot_functions.append(f"   #transfering {component}")
                        self.ot_functions.append(f"   custom_transfer(pipette=pipette,tip_rack=tip_rack, volume={v},source={s},destination={d}, blow_out={bool(self.blowout)}, air_gap={self.air_gap})")
                        #not first or last
                    elif dispense_index > 0 and i < len(source['rows'])-1 and v>10:
                        self.ot_functions.append(f"   #transfering {component}")
                        self.ot_functions.append(f"   custom_transfer(pipette=pipette,tip_rack=tip_rack, volume={v},source={s},destination={d}, blow_out={bool(self.blowout)}, air_gap={self.air_gap})")
                        #last
                    elif v>10:
    ...
    def start_stop_heater_shaker(self, reaction_index):
        if self.fixed_time and self.fixed_temperature:
            ...
            if self.shaker:
                self.ot_functions.append(f"   protocol.delay(minutes={0.5})")
            ...
            self.ot_functions.append(f"   protocol.delay(minutes={self.times[0]-0.5})")
            if self.shaker:
                self.ot_functions.append(f"   hs_mod.deactivate_shaker()")
            self.ot_functions.append(f"   hs_mod.deactivate_heater()")
        elif self.fixed_time and not self.fixed_temperature:
            if self.shaker:
                self.ot_functions.append(f"   hs_mod.set_and_wait_for_shake_speed({self.shaker_speeds[0]})")
            ...
            if self.shaker:
                self.ot_functions.append(f"   hs_mod.deactivate_shaker()")
            self.ot_functions.append(f"   hs_mod.deactivate_heater()")
        elif not self.fixed_time and self.fixed_temperature:
            elapsed_time = 0
            ...
            if self.shaker:
                self.ot_functions.append(f"   hs_mod.deactivate_shaker()")
            self.ot_functions.append(f"   hs_mod.deactivate_heater()")
        elif not self.fixed_time and not self.fixed_temperature:
    ...
    def mix_analysis_sample(self, reaction_i, reaction):
        ...
        #earlier analysis solvent not only solvent!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        analysis_solvent_volume = self.solutions[self.solutions['component'] == 'solvent']['volume'].values[0]
